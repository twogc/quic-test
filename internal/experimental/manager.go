...
func (em *ExperimentalManager) StartServer(ctx context.Context) error {
	...
	var qlogTr *QlogTracer
	var mpMgr *MultipathManager
	if em.ccManager != nil {
		ccMgr = em.ccManager.(*CongestionControlManager)
	}
	if em.qlogTracer != nil {
		qlogTr = em.qlogTracer.(*QlogTracer)
	}
	if em.multipathMgr != nil {
		mpMgr = em.multipathMgr.(*MultipathManager)
	}
	em.server = NewExperimentalServer(em.logger, em.config, em.ackManager, ccMgr, qlogTr, mpMgr, em.fecManager)
...
func (em *ExperimentalManager) StartClient(ctx context.Context) error {
	...
	em.client = NewExperimentalClient(em.logger, em.config, em.ackManager, ccMgr, qlogTr, mpMgr, em.fecManager)
...
func (em *ExperimentalManager) initializeComponents(ctx context.Context) error {
	...
	if em.ccManager != nil {
		ccMgr = em.ccManager.(*CongestionControlManager)
		if ccMgr != nil && ccMgr.GetSendController() != nil {
			em.ccIntegration = metrics.NewCCIntegration(em.prometheusMetrics, ccMgr.GetSendController())
...
func (em *ExperimentalManager) stop() {
	...
	if em.ccManager != nil {
		if ccMgr, ok := em.ccManager.(*CongestionControlManager); ok {
			ccMgr.Stop()
		}
	}
	
	if em.qlogTracer != nil {
		if qlogTr, ok := em.qlogTracer.(*QlogTracer); ok {
			qlogTr.Close()
		}
	}
	
	if em.multipathMgr != nil {
		if mpMgr, ok := em.multipathMgr.(*MultipathManager); ok {
			mpMgr.Stop()
		}
	}
...
func (em *ExperimentalManager) GetMetrics() map[string]interface{} {
	...
	if em.ccManager != nil {
		if ccMgr, ok := em.ccManager.(*CongestionControlManager); ok {
			metrics["congestion_control"] = ccMgr.GetMetrics()
		}
	}
	
	if em.multipathMgr != nil {
		if mpMgr, ok := em.multipathMgr.(*MultipathManager); ok {
			metrics["multipath"] = mpMgr.GetMetrics()
		}
	}
...
func (em *ExperimentalManager) setupQlogIntegration() {
	ccMgr, ok := em.ccManager.(*CongestionControlManager)
	if !ok || em.qlogTracer == nil || ccMgr == nil || ccMgr.GetSendController() == nil {
		return
	}
